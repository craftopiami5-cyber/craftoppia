...
function switchMainTab(tab) {
  ...
  if (tab !== 'simulator' && simLogsInterval) {
    clearInterval(simLogsInterval);
    simLogsInterval = null;
  }
  ...
  if (tab === 'submissions') {
    ...
    document.getElementById("sectionSimulator").classList.add("active");
    initSimulatorTab();
  }
}
...
// Bot Simulator UI logic
let simLogsInterval = null;
let simulatedHistory = [];

function initSimulatorTab() {
  renderSimHistory();
  if (simLogsInterval) clearInterval(simLogsInterval);
  simLogsInterval = setInterval(fetchSimLogs, 1500);
}

async function fetchSimLogs() {
  const chatId = document.getElementById("simChatId").value;
  if (!chatId) return;
  try {
    const res = await fetch(`/api/bot/simulator-logs?chat_id=${chatId}`);
    if (res.ok) {
      const logs = await res.json();
      let updated = false;
      for (const log of logs) {
        const exists = simulatedHistory.some(h => h.isBot && h.timestamp === log.timestamp && h.text === log.payload.text);
        if (!exists && log.payload.text) {
          simulatedHistory.push({
            isBot: true,
            text: log.payload.text,
            timestamp: log.timestamp,
            reply_markup: log.payload.reply_markup
          });
          updated = true;
        }
      }
      if (updated) {
        simulatedHistory.sort((a,b) => new Date(a.timestamp) - new Date(b.timestamp));
        renderSimHistory();
      }
    }
  } catch(e) {
...
async function sendSimulatedText(text) {
  const chatId = document.getElementById("simChatId").value;
  if (!chatId) return;
  ...
  try {
    await fetch("/api/bot/simulate-message", {
      method: "POST",
...
async function sendSimulatedCustomText() {
  const input = document.getElementById("simMessageText");
  const text = input.value.trim();
  if (!text) return;
  input.value = "";
  await sendSimulatedText(text);
}

async function sendSimulatedContact() {
  const chatId = document.getElementById("simChatId").value;
  const name = document.getElementById("simContactName").value.trim();
  const phone = document.getElementById("simContactPhone").value.trim();
  if (!chatId || !phone) return;
  
  simulatedHistory.push({
    isBot: false,
    text: `Shared Contact: ${name} (${phone})`,
    timestamp: new Date().toISOString()
  });
  renderSimHistory();
  ...
  try {
    await fetch("/api/bot/simulate-message", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ chat_id: chatId, phone: phone, name: name })
    });
  } catch(e) {
    console.error(e);
  }
...
async function clickSimCallback(callbackData) {
  const chatId = document.getElementById("simChatId").value;
  simulatedHistory.push({
    isBot: false,
    text: `[Clicked Button]: ${callbackData}`,
    timestamp: new Date().toISOString()
  });
  renderSimHistory();
  try {
    await fetch("/api/bot/simulate-message", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ chat_id: chatId, callback_data: callbackData })
    });
  } catch(e) {
    console.error(e);
  }
...
function renderSimHistory() {
  const container = document.getElementById("simChatHistory");
  if (!container) return;
  
  if (simulatedHistory.length === 0) {
    container.innerHTML = `<div style="color:var(--text-muted); text-align:center; margin-top:50px; font-size:14px;">No messages yet. Send a message to start!</div>`;
    return;
  }
  
  container.innerHTML = simulatedHistory.map(h => {
    const isBot = h.isBot;
    const align = isBot ? "left" : "right";
    const bg = isBot ? "rgba(255,255,255,0.08)" : "var(--primary)";
    const color = "#fff";
    const border = isBot ? "border-top-left-radius: 4px;" : "border-top-right-radius: 4px;";
    
    let markupHtml = "";
    if (isBot && h.reply_markup) {
      let kb = h.reply_markup;
      if (typeof kb === 'string') {
        try { kb = JSON.parse(kb); } catch(e) {}
      }
      if (kb && kb.inline_keyboard) {
        markupHtml = `<div style="display:flex; flex-direction:column; gap:6px; margin-top:10px;">`;
        kb.inline_keyboard.forEach(row => {
          markupHtml += `<div style="display:flex; gap:6px;">`;
          row.forEach(btn => {
            markupHtml += `<button class="btn-logout" style="background:rgba(255,255,255,0.15); color:#fff; border:none; padding:6px 10px; font-size:11px; flex:1; border-radius:6px; cursor:pointer;" onclick="clickSimCallback('${btn.callback_data}')">${btn.text}</button>`;
          });
          markupHtml += `</div>`;
        });
        markupHtml += `</div>`;
    ...
    let cleanText = escapeHtml(h.text || "")
      .replace(/\*\*(.*?)\*\*/g, '<b>$1</b>')
      .replace(/\*(.*?)\*/g, '<i>$1</i>')
    ...
    return `
      <div style="display:flex; justify-content:${isBot ? 'flex-start' : 'flex-end'}; width:100%;">
        <div style="background:${bg}; color:${color}; padding:12px 16px; border-radius:18px; ${border} max-width:80%; box-shadow: 0 4px 10px rgba(0,0,0,0.15);">
          <div style="font-size:12px; color:rgba(255,255,255,0.5); margin-bottom:4px; font-weight:700;">${isBot ? 'BOT' : 'YOU'}</div>
...
async function sendSimulatedPhoto() {
  const chatId = document.getElementById("simChatId").value;
  const fileInput = document.getElementById("simPhotoFile");
  if (!chatId) {
    alert("Please select or enter a Mock Chat ID first.");
    return;
  }
  ...
  try {
    ...
    if (res.ok && data.url) {
      simulatedHistory.push({
        isBot: false,
        text: `🖼️ [Sent Photo]: ${fileInput.files[0].name}`,
        timestamp: new Date().toISOString()
      });
      renderSimHistory();
      ...
      await fetch("/api/bot/simulate-message", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ chat_id: chatId, photo_url: data.url })
      });
      
      fileInput.value = "";
    } else {
      alert("Failed to upload simulated photo: " + (data.message || "Unknown error"));
    }
  } catch (e) {
    console.error(e);
    alert("Error uploading simulated photo.");
  }
}
