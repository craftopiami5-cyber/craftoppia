...
async function generateCertificatePdf(name: string, regDate: string, finishDate: string, name2?: string): Promise<Uint8Array> {
  ...
  return new Promise((resolve) => {
    ...
    doc.moveTo(417.5, 185).lineTo(417.5, 435).stroke();
    doc.lineWidth(2).strokeColor(antiqueGold);
    doc.moveTo(421.5, 185).lineTo(421.5, 435).stroke();
    ...
    doc.fillColor(forestGreen).font(ethFont(false)).fontSize(12).text("ለ", lx + 20, 206);
    doc.fillColor(forestGreen).font(autoFont(name, true)).fontSize(13)
       .text(name, lx + 40, 202, { width: lw - 40, align: "center" });
    doc.moveTo(lx + 35, 218).lineTo(lx + lw, 218).strokeColor(forestGreen).lineWidth(1).stroke();

    // Line 2: በክራፍቶፒያ የእደጥበብ ማሰልጠኛ ተቋም _____
    doc.fillColor(forestGreen).font(ethFont(false)).fontSize(11).text("በክራፍቶፒያ የእደጥበብ ማሰልጠኛ ተቋም", lx, 240);
    doc.moveTo(lx + 215, 252).lineTo(lx + lw, 252).strokeColor(forestGreen).lineWidth(1).stroke();
    doc.fillColor(forestGreen).font(autoFont(durationAm, true)).fontSize(11)
       .text(durationAm, lx + 215, 238, { width: lw - 215, align: "center" });

    // Line 3: ሳምንት ለተሰጠው የ _____
    doc.fillColor(forestGreen).font(ethFont(false)).fontSize(11).text("ሳምንት ለተሰጠው የ", lx, 275);
    doc.moveTo(lx + 105, 287).lineTo(lx + lw, 287).strokeColor(forestGreen).lineWidth(1).stroke();
    doc.fillColor(forestGreen).font(autoFont(programAm, true)).fontSize(11)
       .text(programAm, lx + 105, 273, { width: lw - 105, align: "center" });

    // Line 4: ሙያ ስልጠና ተከታትሎ(ላ) በስኬት ላጠናቀቀ(ች) ይህ የምስክር ወረቀት ተሰጥቶታል(ታለች)፡፡
    doc.fillColor(forestGreen).font(ethFont(false)).fontSize(11)
       .text("ሙያ ስልጠና ተከታትሎ(ላ) በስኬት ላጠናቀቀ(ች) ይህ የምስክር ወረቀት ተሰጥቶታል(ታለች)፡፡", lx, 310, { width: lw, align: "justify", lineGap: 6 });
    ...
    doc.fillColor(forestGreen).font(latFont(false)).fontSize(10.5)
       .text("THIS CERTIFICATE IS PROUDLY PRESENTED FOR", rx, 240);
    doc.text("SUCCESSFULLY COMPLETING A SHORT-TERM TRAINING", rx, 262);

    // Line 4: PROGRAM IN _______
    doc.text("PROGRAM IN", rx, 284);
    doc.moveTo(rx + 75, 296).lineTo(rx + rw, 296).strokeColor(forestGreen).lineWidth(1).stroke();
    doc.fillColor(forestGreen).font(autoFont(programEn, true)).fontSize(10.5)
       .text(programEn.toUpperCase(), rx + 75, 282, { width: rw - 75, align: "center" });

    // Line 5: AT CRAFTOPIA.
    doc.fillColor(forestGreen).font(latFont(false)).fontSize(10.5).text("AT CRAFTOPIA.", rx, 308);

    // Line 6: THE TRAINING WAS CONDUCTED FOR _____ WEEK.
    doc.text("THE TRAINING WAS CONDUCTED FOR", rx, 330);
    doc.moveTo(rx + 195, 342).lineTo(rx + 270, 342).strokeColor(forestGreen).lineWidth(1).stroke();
    doc.fillColor(forestGreen).font(autoFont(durationEn, true)).fontSize(10.5)
       .text(durationEn, rx + 195, 328, { width: 75, align: "center" });
    doc.fillColor(forestGreen).font(latFont(false)).text("WEEK.", rx + 275, 330);
    ...
    if (settings.seal_base64) {
      try {
        ...
        doc.image(sealBuf, rx + 40, 370, { fit: [140, 140] });
      } catch (sealErr: any) {
